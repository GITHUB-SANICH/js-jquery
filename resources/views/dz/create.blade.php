...
<a href="/" class="back-button">На главную</a>
	...
	{!! Form::open(['class' => 'article-form']) !!}
		...
		{{Form::text('title', '', ['placeholder' => 'Введите название товара', 'type' => 'text'])}}

		{{Form::label('description', 'Описание товара')}}
		{{Form::textarea('description', '', ['placeholder' => 'Опишите товар', 'id' => 'editor'])}}

		{{Form::label('category', 'Категория товара')}}
		{{Form::text('category', '', ['placeholder' => 'Введите категорию товара'])}}

		{{Form::label('price', 'Цена товара')}}
		{{Form::text('price', '', ['placeholder' => 'Введите стоимость товара (руб.)'])}}

		{{Form::submit('Добавить товар', ['class' => 'add-button'])}}
