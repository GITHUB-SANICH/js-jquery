	...
	<div class="h-screen bg-white flex flex-col space-y-10 justify-center items-center">
		<div class="bg-white w-96 shadow-xl rounded p-5">
			 ...
			 <form action="{{ route("register_process") }}" method="POST" class="space-y-5 mt-5">
				@csrf
					<input name="name" type="text" class="w-full h-12 border border-gray-800 @error('name') border-red-500 @enderror rounded px-3" placeholder="Имя" />
					@error('name')
				  ...
				  <input	name="password" type="password" class="w-full h-12 border border-gray-800 @error('password') border-red-500 @enderror rounded px-3" placeholder="Пароль" />
						@error('password')
							<p class="test-red-500">{{ $message }}</p>
						@enderror				
				  <input	name="password_confirmation" type="password" class="w-full h-12 border border-gray-800 @error('password_confirmation') border-red-500 @enderror rounded px-3" placeholder="Подтверждение пароля" />
