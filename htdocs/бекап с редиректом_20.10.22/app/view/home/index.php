	...
	<div class="container main">
		...
		<?php if($_COOKIE['login'] == ''): ?>
			<div class="form-auth">
				...
				<form action="/home/reg" method="POST" class="form-control">
		...
		<?php else: ?>
			<div class="form-auth">
				...
				<form action="/home/link" method="POST" class="form-control">
					...
					<!-- блок с выводом ошибок -->
					<div class="error"><?=$data['message_1']?></div>
					<div class="error"><?=$data['message']?></div>
					<!-- кнопка отправки формы -->
		...
		<?php if($_COOKIE['login'] != '' && $data['links'] != ''): ?>
			<!-- вывод списка ссылок -->	
			<div class="list_links">
			<h2>Сокращенные ссылки</h2>
			<?php foreach ($data['links'] as $links): ?>		
				<div class="shortLinkClass" style="padding-bottom: 15px;">
					<p><b>Длинная:</b> <?=$links['long_link']?></p>
					<p><b>Короткая:</b> <a href="/short/<?=$links['short_link']?>"><?=$_SERVER['HTTP_HOST'].'/short/'.$links['short_link']?></a></p>
					<form action="/home/dropShort/" method="POST">
						<input type="hidden" name="deleteLink" value="<?=$links['short_link']?>">
						<button type="submit" class="btn">Удалить ссылку <i class="fa fa-trash" aria-hidden="true"></i></button>
					</form>
				</div>
			<?php endforeach; ?>	
			</div>
